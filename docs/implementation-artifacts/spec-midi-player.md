...
- Given `bun run midi-player.js`, when run, then it exits 0 (the script loads and parses; no Scripter globals are invoked at top level).
- Given `bun test`, when run, then all `eventsInBlock` tests pass.
...
**Commands:**
- `bun run midi-player.js` -- expected: exit 0. Loads and parses the script; no Scripter globals are invoked at top level, so it returns cleanly under bun.
- `bun test` -- expected: all tests pass; exercises the `eventsInBlock` edge cases from the I/O matrix.
...
- `midi-player.test.js` — 10 `bun:test` unit tests for `eventsInBlock` (mid-loop, half-open range, loop-boundary span, multi-loop span, empty/zero-width blocks, `offBeat === onBeat + length`, non-positive `loopBeats`, fractional edges, out-of-range/negative offset normalization).
...
**Verification:** `bun run midi-player.js` → exit 0; `bun test` → 10/10 pass, 59 assertions. Manual in-MainStage checks (host sync, tempo tracking, metronome-strip block-notes, no hung notes on stop) remain for Michael at the keyboard — Scripter cannot run outside Logic/MainStage.
